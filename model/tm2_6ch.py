...
class UNetBlock(nn.Module):
    ...
    def forward(self, x, attn=None):
        ...
        if attn is not None:
            B, C, H, T = x.shape
            x_flat = x.permute(0, 2, 1, 3).reshape(B, H, C * T)  # [B, H, C*T]
            x_attn = torch.bmm(attn, x_flat).view(B, H, C, T).permute(0, 2, 1, 3)
            x = x + (x * x_attn)
        return x
...
class SharedRowAttentionUNet(nn.Module):
    def __init__(self, in_channels=3):
        ...
        # YUV decoder (for attention only, not fused)
        ...
        # Decoder (RGB + YUV skip connection)
        self.up2 = nn.ConvTranspose2d(256, 128, kernel_size=2, stride=2)
        self.dec2 = UNetBlock(256 + 128, 128)  # f2 + y2 + upsample
        self.up1 = nn.ConvTranspose2d(128, 64, kernel_size=2, stride=2)
        self.dec1 = UNetBlock(128 + 64, 64)   # f1 + y1 + upsample
    ...
    def forward(self, x_rgb, x_yuv, epoch=None):
        ...
        # Decoder with YUV fusion
        d2 = self.up2(f3)
        d2 = torch.cat([d2, f2, y2], dim=1)
        d2 = self.dec2(d2, a2)

        d1 = self.up1(d2)
        d1 = torch.cat([d1, f1, y1], dim=1)
        d1 = self.dec1(d1, a1)
        ...
        attn_weight = attn_weight.unsqueeze(1).unsqueeze(-1)  # [B, 1, H, 1]

        out_weighted = (out * attn_weight).sum(dim=2)  # [B, C, T]
        out_temporal = self.temporal_head(out_weighted)  # [B, 1, T]

        # Residual Fusion
        out = out_temporal + out_weighted.mean(dim=1, keepdim=True)  # [B, 1, T]
