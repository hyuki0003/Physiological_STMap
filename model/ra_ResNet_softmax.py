...
from .rowAttn import RowAttentionSoftmax
...
class RowAttentionUNet(nn.Module):
    def __init__(self, in_channels=3):
        ...
        # Row-wise Attention Extractors
        self.attn1 = RowAttentionSoftmax(64)
        self.attn2 = RowAttentionSoftmax(128)
        self.attn3 = RowAttentionSoftmax(256)
    ...
    def forward(self, x_rgb, x_yuv, epoch=None):
        ...
        # Row attention-weighted sum: [B, 1, H, 1] × [B, H, T] → [B, T]
        final_attn = self.attn3(y3)       # [B, 1, H, 1]
        final_attn = F.interpolate(final_attn, size=out.shape[1:], mode='bilinear', align_corners=False)  # [B, 1, H, T]
        final_attn = final_attn.squeeze(1)  # [B, H, T]
        final_attn = torch.softmax(final_attn, dim=1)  # row-wise softmax

        output = (out * final_attn).sum(dim=1)  # weighted sum over rows → [B, T]
        return output
