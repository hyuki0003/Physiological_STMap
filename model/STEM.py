...
# ---------------- Directional Stem Block ----------------

class DirectionalStem(nn.Module):
    def __init__(self, in_ch, out_ch):
        ...
        self.conv_fuse = nn.Conv2d(out_ch, out_ch, kernel_size=3, padding=1)
        self.bn = nn.BatchNorm2d(out_ch)
        self.relu = nn.ReLU(inplace=True)

    def forward(self, x):
        h_feat = self.conv_h(x)  # (B, out_ch//2, H, T)
        v_feat = self.conv_v(x)  # (B, out_ch//2, H, T)
        feat = torch.cat([h_feat, v_feat], dim=1)  # (B, out_ch, H, T)
        return self.relu(self.bn(self.conv_fuse(feat)))
...
class TransfuserSTMapModel(nn.Module):
    def __init__(self, ch_in=3, base_dim=64):
        super().__init__()
        # Directional stem before encoders
        self.rgb_stem = DirectionalStem(ch_in, ch_in)
        self.yuv_stem = DirectionalStem(ch_in, ch_in)

        # Deep encoders
        self.rgb_encoder = DeepEncoder(ch_in, base_ch=base_dim)
        self.yuv_encoder = DeepEncoder(ch_in, base_ch=base_dim)

        # Decoder will be initialized lazily
        self.temporal_decoder = None

    def forward(self, stmap_rgb, stmap_yuv):
        stmap_rgb = self.rgb_stem(stmap_rgb)  # (B, 3, H, T)
        stmap_yuv = self.yuv_stem(stmap_yuv)
